...
def test_model1_model2_onnx(ort_session1, ort_session2, path_list, in_folder, sav_folder, use_gpu_for_torch_ops):
    ...
    for im_path in tqdm(path_list):
        ...
        # pred_onnx có shape (1, C, H, W), C=3
        pred_processed = np.transpose(pred_onnx[0], (1, 2, 0))
        # pred_processed = pred_processed.astype(np.float32) // <============================
...
if __name__ == '__main__':
    ...
    model1_onnx_path = os.path.join(onnx_model_dir, 'gcnet_fp32.onnx')
    model2_onnx_path = os.path.join(onnx_model_dir, 'drnet_fp16.onnx')

    img_folder = './distorted/'
    sav_folder = './enhanced_onnx_/' if use_onnx else './enhanced_pytorch/'
    ...
    if not os.path.exists(model1_onnx_path):
        ...
        model1_pt = UNext_full_resolution_padding(num_classes=3, input_channels=3, img_size=512).to(export_device)
        ...
        torch.onnx.export(model1_pt,
                          ...
                          dynamic_axes={'input_1': {2: 'height', 3: 'width'}, 
                                        'output_1': {2: 'height', 3: 'width'}},
                          ) 
    ...
    if not os.path.exists(model2_onnx_path):
        ...
        model2_pt = UNext_full_resolution_padding_L_py_L(num_classes=3, input_channels=6, img_size=512).to(export_device)
        ...
        torch.onnx.export(model2_pt,
                          ...
                          dynamic_axes={'input_2': {2: 'height', 3: 'width'},
                                        'pred_output': {2: 'height', 3: 'width'}, 
                                        'aux_output1': {2: 'height', 3: 'width'}, 
                                        'aux_output2': {2: 'height', 3: 'width'},
                                        'aux_output3': {2: 'height', 3: 'width'}},
                          )
    ...
    if use_onnx:
        ...
        if not use_cpu and onnxruntime.get_device() == 'GPU':             
            if torch.cuda.is_available():
                ...
                print("Warning: GPU được yêu cầu nhưng PyTorch/CUDA không khả dụng. Sẽ sử dụng CPU.")
        else:
        ...
        model1 = UNext_full_resolution_padding(num_classes=3, input_channels=3, img_size=512).to(run_device_pytorch)
        state = convert_state_dict(torch.load(model1_pkl_path, map_location=torch.device('cpu'))['model_state'])    
        model1.load_state_dict(state)
        model2 = UNext_full_resolution_padding_L_py_L(num_classes=3, input_channels=6, img_size=512).to(run_device_pytorch)
