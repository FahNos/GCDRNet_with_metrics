...
# --- Hàm inference bằng ONNX (hỗ trợ FP16) ---
def test_model1_model2_onnx(ort_session1, ort_session2, path_list, in_folder, sav_folder, use_fp16=False):
    ...
    for im_path in tqdm(path_list):
        ...
        shadow_torch_fp32 = torch.from_numpy(shadow_onnx.astype(np.float32))
        shadow_resized_torch_fp32 = F.interpolate(shadow_torch_fp32, (h_padded, w_padded), mode='bilinear', align_corners=False)
        shadow_resized_np = shadow_resized_torch_fp32.numpy().astype(dtype) 

        model1_im_np = np.clip(im_org_torch_like_np / (shadow_resized_np + 1e-6), 0, 1) 
        ...
        shadow_processed_display = np.transpose(shadow_resized_np[0], (1,2,0)) # (H, W, C)
        shadow_processed_display = (shadow_processed_display.astype(np.float32) * 255).astype(np.uint8)
        shadow_processed_display = shadow_processed_display[padding_h:, padding_w:]

        model1_im_processed_display = np.transpose(model1_im_np[0], (1,2,0)) # (H, W, C)
        model1_im_processed_display = (model1_im_processed_display.astype(np.float32) * 255).astype(np.uint8)
        model1_im_processed_display = model1_im_processed_display[padding_h:, padding_w:]

        pred_processed = np.transpose(pred_onnx[0], (1, 2, 0)) # (H, W, C)
...
def optimize_onnx_model(original_model_path, optimized_model_path, providers):
    """
    Optimizes an ONNX model and saves the optimized version.
    """
    print(f"Optimizing ONNX model: {original_model_path} -> {optimized_model_path}")
    sess_options = ort.SessionOptions()
    sess_options.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL
    sess_options.optimized_model_filepath = optimized_model_path
    
    _ = ort.InferenceSession(original_model_path, sess_options, providers=providers)
...
if __name__ == '__main__':
    ...
    onnx_suffix = "_fp16" if USE_FP16_ONNX else ""
    optimized_suffix = "_optimized"

    model1_onnx_original_path = os.path.join(onnx_model_dir, f'gcnet{onnx_suffix}.onnx')
    model2_onnx_original_path = os.path.join(onnx_model_dir, f'drnet{onnx_suffix}.onnx')

    model1_onnx_optimized_path = os.path.join(onnx_model_dir, f'gcnet{onnx_suffix}{optimized_suffix}.onnx')
    model2_onnx_optimized_path = os.path.join(onnx_model_dir, f'drnet{onnx_suffix}{optimized_suffix}.onnx')

    img_folder = './distorted/'
    sav_folder = f'./enhanced_onnx{"_fp16" if USE_FP16_ONNX else ""}_optimized_test/'
    ...
    # Model 1
    if not os.path.exists(model1_onnx_original_path):
        ...
        torch.onnx.export(model1_pt,
                          dummy_input1,
                          model1_onnx_original_path,
                          input_names=['input_1'],
                          output_names=['output_1'],
                          dynamic_axes={'input_1': {0: 'batch', 2: 'height', 3: 'width'},
                                        'output_1': {0: 'batch', 2: 'height', 3: 'width'}},
                          opset_version=12)
        print(f"Model 1 (gốc) đã được lưu vào {model1_onnx_original_path}")
        del model1_pt

    # Model 2
    if not os.path.exists(model2_onnx_original_path):
        ...
        torch.onnx.export(model2_pt,
                          dummy_input2,
                          model2_onnx_original_path,
                          input_names=['input_2'],
                          output_names=output_names_model2,
                          dynamic_axes=dynamic_axes_model2,
                          opset_version=12)
        print(f"Model 2 (gốc) đã được lưu vào {model2_onnx_original_path}")
        del model2_pt
    ...
    if torch.cuda.is_available() and ort.get_device() == 'GPU':
        ...
        print("Sử dụng CPUExecutionProvider.")


    # --- TỐI ƯU HÓA MODEL ONNX (NẾU FILE ĐÃ TỐI ƯU CHƯA CÓ) ---
    # Model 1
    if not os.path.exists(model1_onnx_optimized_path):
        if not os.path.exists(model1_onnx_original_path):
             raise FileNotFoundError(f"Original ONNX model not found: {model1_onnx_original_path}. Please run PyTorch to ONNX conversion first.")
        optimize_onnx_model(model1_onnx_original_path, model1_onnx_optimized_path, providers)
    else:
        print(f"Sử dụng model đã tối ưu: {model1_onnx_optimized_path}")

    # Model 2
    if not os.path.exists(model2_onnx_optimized_path):
        if not os.path.exists(model2_onnx_original_path):
            raise FileNotFoundError(f"Original ONNX model not found: {model2_onnx_original_path}. Please run PyTorch to ONNX conversion first.")
        optimize_onnx_model(model2_onnx_original_path, model2_onnx_optimized_path, providers)
    else:
        print(f"Sử dụng model đã tối ưu: {model2_onnx_optimized_path}")


    # --- TẠO SESSION VỚI MODEL ĐÃ TỐI ƯU VÀ TẮT OPTIMIZATION CỦA SESSION ---
    print(f"Tải model đã tối ưu và chạy inference bằng ONNX Runtime ({'FP16' if USE_FP16_ONNX else 'FP32'})...")
    
    sess_options_infer = ort.SessionOptions()
    sess_options_infer.graph_optimization_level = ort.GraphOptimizationLevel.ORT_DISABLE_ALL
    
    # Load model đã tối ưu
    ort_session1 = ort.InferenceSession(model1_onnx_optimized_path, sess_options_infer, providers=providers)
    ort_session2 = ort.InferenceSession(model2_onnx_optimized_path, sess_options_infer, providers=providers)
